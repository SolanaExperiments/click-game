...
using ClickGame;
using ClickGame.Program;
using ClickGame.Errors;
using ClickGame.Accounts;
...
namespace ClickGame
{
    ...
    namespace Errors
    {
        public enum ClickGameErrorKind : uint
        {
    ...
    public partial class ClickGameClient : TransactionalBaseClient<ClickGameErrorKind>
    {
        public ClickGameClient(IRpcClient rpcClient, IStreamingRpcClient streamingRpcClient, PublicKey programId) : base(rpcClient, streamingRpcClient, programId)
        {
        }
        ...
        {
            Solana.Unity.Rpc.Models.TransactionInstruction instr = Program.ClickGameProgram.InitPlayer(accounts, levelSeed, programId);
        ...
        {
            Solana.Unity.Rpc.Models.TransactionInstruction instr = Program.ClickGameProgram.ChopTree(accounts, levelSeed, counter, programId);
            return await SignAndSendTransaction(instr, feePayer, signingCallback);
        }
        ...
        {
            Solana.Unity.Rpc.Models.TransactionInstruction instr = Program.ClickGameProgram.SuperChopTree(accounts, levelSeed, counter, programId);
            return await SignAndSendTransaction(instr, feePayer, signingCallback);
        }

        protected override Dictionary<uint, ProgramError<ClickGameErrorKind>> BuildErrorsDictionary()
        {
            return new Dictionary<uint, ProgramError<ClickGameErrorKind>>{{6000U, new ProgramError<ClickGameErrorKind>(ClickGameErrorKind.NotEnoughEnergy, "Not enough energy")}, {6001U, new ProgramError<ClickGameErrorKind>(ClickGameErrorKind.WrongAuthority, "Wrong Authority")}, };
        }
    }
    ...
    namespace Program
    {
        ...
        public static class ClickGameProgram
        {
